...
void initPipe(int pipe_, int pipeLen){
    if(pipeLen > 1){
        if(pipe_ != FIRST_PROCESS){
        pfds_prev = pfds_next;                             // ready to take input from pipe
        }
        if(pipe_ != LAST_PROCESS){                                 // syntax error when pipelen > 1 and myargs[0]
...
int *connectToInputPipe(int pipe_, int pipeLen){
...
int *connectToOutputPipe(int pipe_, int pipeLen){
...
void closeParentPipe(int pipe_, int pipeLen){
    if(pipeLen > 1 && pipe_ != FIRST_PROCESS){
        close(pfds_prev[0]); close(pfds_prev[1]); 
        free(pfds_prev);
    }
